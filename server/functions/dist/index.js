...
app.get('/api/persons', (req, res) => {
    try {
        const data = (0, persons_1.getPersons)();
...
app.post('/api/person', jsonMiddleware, (req, res) => {
    try {
        (0, persons_1.addPerson)(req.body);
        res.set('Access-Control-Allow-Origin', '*');
...
app.get('/api/cities/:countryId', (req, res) => {
    try {
        const cities = (0, countries_1.getCitiesByCountryId)(+req.params.countryId);
        res.set('Access-Control-Allow-Origin', '*');
...
app.post('/api/city', jsonMiddleware, (req, res) => {
    try {
        (0, countries_1.addCity)(req.body);
        res.set('Access-Control-Allow-Origin', '*');
...
app.get('/api/countries', (req, res) => {
    try {
        const data = (0, countries_1.getCountries)();
        res.set('Access-Control-Allow-Origin', '*');
        res.status(200).json(data);
    }
    catch (error) {
